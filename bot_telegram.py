...
class TelegramBot():
    ...
    async def iniciar_captura(self,update:Update, context:CallbackContext):
        err_msg = "Error al iniciar la captura de imagenes, argumentos inválidos\n" \
                    ...
                    "/iniciar_captura --intervalo <n> --volumen <porcentaje de aviso>\n" \
                    "Donde <n> es el intervalo de tiempo en minutos y <porcentaje de aviso> es el porcentaje de volumen que debe de tener el pan para mandar un aviso\n" \
                    "Ejemplo: /iniciar_captura --intervalo 5 --volumen 50"
        ...
        if context.args:
            if len(context.args) == 4:    
                ...
                if (context.args[0] == "--intervalo" and context.args[2] == "--volumen"):

                    self.capturadora.capturando = True
                    self.capturadora.interval = int(context.args[1])

                    self.capturadora.volumen = int(context.args[3])

                    logging.info(f"Captura iniciada con intervalo de {self.capturadora.interval} minutos y volumen de aviso {self.capturadora.volumen}%")
                    await update.message.reply_text(f"Iniciando captura de imagenes cada {self.capturadora.interval} minutos, se avisará cuando el volumen de la imagen sea mayor que {self.capturadora.volumen}%") 
    ...
    async def detener_captura(self,update:Update, context:CallbackContext):
        ...
        self.capturadora.primera_elipse = None
    ...
    async def status(self,update:Update, context:CallbackContext):
        ...
        if(self.capturadora.capturando):
            ...
            f"Crecimiento actual: {self.capturadora.crecimiento}%\n" \
            f"Aviso al alcanzar el {self.capturadora.volumen}% de volumen\n" \
    ...
    async def help(self,update:Update, context:CallbackContext):
        ...
        "/iniciar_captura --intervalo <n> --volumen <porcentaje de aviso>: Inicia la captura de imagenes automaticas\n" \
